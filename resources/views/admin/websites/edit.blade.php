...
<div class="space-y-6">
    <div>
        <h2 class="text-2xl font-semibold text-slate-900">Edit Website</h2>
        <p class="text-sm text-slate-600">Update ownership, status, and metadata.</p>
    </div>
    ...
    <form method="POST" action="/admin/websites/{{ $website->id }}" class="space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <select name="user_id" class="mt-1 w-full rounded-lg border-slate-300 focus:border-slate-900 focus:ring-slate-900">
                    @foreach($clients as $client)
                        <option value="{{ $client->id }}"
                            {{ (old('user_id', $website->user_id) == $client->id) ? 'selected' : '' }}>
                            {{ $client->name }}
                        </option>
                    @endforeach
                </select>
                ...
                <select name="country" class="mt-1 w-full rounded-lg border-slate-300 focus:border-slate-900 focus:ring-slate-900">
                    ...
                    @foreach($countries as $country)
                        <option value="{{ $country }}"
                            {{ (old('country', $website->country) == $country) ? 'selected' : '' }}>
                            {{ $country }}
                        </option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <select name="niche" class="mt-1 w-full rounded-lg border-slate-300 focus:border-slate-900 focus:ring-slate-900">
                    ...
                    @foreach($niches as $niche)
                        <option value="{{ $niche }}"
                            {{ (old('niche', $website->niche) == $niche) ? 'selected' : '' }}>
                            {{ $niche }}
                        </option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <select name="status" class="mt-1 w-full rounded-lg border-slate-300 focus:border-slate-900 focus:ring-slate-900">
                    ...
                    <option value="paused" {{ old('status', $website->status) === 'paused' ? 'selected' : '' }}>Paused</option>
                </select>
            </div>
        </div>
