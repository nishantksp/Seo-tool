...
<div class="space-y-6">
    <div>
        <h2 class="text-2xl font-semibold text-slate-900">Edit Keyword Assignment</h2>
        <p class="text-sm text-slate-600">Update target URL, status, and keyword metrics.</p>
    </div>
    ...
    <form method="POST" action="/admin/keywords/{{ $keyword->id }}" class="space-y-8">
        @csrf
        @method('PUT')
        ...
        <div class="bg-white border border-slate-200 rounded-xl p-6 space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <select name="website_id" class="mt-1 w-full rounded-lg border-slate-300 focus:border-slate-900 focus:ring-slate-900">
                        @foreach($websites as $site)
                            <option value="{{ $site->id }}" {{ old('website_id', $keyword->website_id) == $site->id ? 'selected' : '' }}>
                ...
                <div>
                    ...
                    <input type="text" name="target_url" value="{{ old('target_url', $keyword->target_url) }}"
                ...
                <div>
                    ...
                    <input type="number" name="priority" min="1" max="10" value="{{ old('priority', $keyword->priority) }}"
                           class="mt-1 w-full rounded-lg border-slate-300 focus:border-slate-900 focus:ring-slate-900">
                </div>
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="notes" rows="3"
                              class="mt-1 w-full rounded-lg border-slate-300 focus:border-slate-900 focus:ring-slate-900">{{ old('notes', $keyword->notes) }}</textarea>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white border border-slate-200 rounded-xl p-6 space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div class="md:col-span-2">
                    ...
                    <input type="text" name="keyword" value="{{ old('keyword', $keyword->keyword->keyword) }}"
                           class="mt-1 w-full rounded-lg border-slate-300 focus:border-slate-900 focus:ring-slate-900">
                </div>
                ...
                <div>
                    ...
                    <select name="intent" class="mt-1 w-full rounded-lg border-slate-300 focus:border-slate-900 focus:ring-slate-900">
                        ...
                        <option value="informational" {{ old('intent', $keyword->keyword->intent) === 'informational' ? 'selected' : '' }}>Informational</option>
                        <option value="transactional" {{ old('intent', $keyword->keyword->intent) === 'transactional' ? 'selected' : '' }}>Transactional</option>
                        <option value="navigational" {{ old('intent', $keyword->keyword->intent) === 'navigational' ? 'selected' : '' }}>Navigational</option>
                    </select>
                </div>

                <div>
                    <label class="block text-sm font-medium text-slate-700">Search Volume</label>
                    <input type="number" name="search_volume" value="{{ old('search_volume', $keyword->keyword->search_volume) }}"
                           class="mt-1 w-full rounded-lg border-slate-300 focus:border-slate-900 focus:ring-slate-900">
                </div>

                <div>
                    <label class="block text-sm font-medium text-slate-700">Difficulty</label>
                    <input type="number" name="difficulty" value="{{ old('difficulty', $keyword->keyword->difficulty) }}"
                           class="mt-1 w-full rounded-lg border-slate-300 focus:border-slate-900 focus:ring-slate-900">
                </div>

                <div>
                    <label class="block text-sm font-medium text-slate-700">Language</label>
                    <input type="text" name="language" value="{{ old('language', $keyword->keyword->language ?? 'en') }}"
                           class="mt-1 w-full rounded-lg border-slate-300 focus:border-slate-900 focus:ring-slate-900">
                </div>

                <div>
                    <label class="block text-sm font-medium text-slate-700">Country</label>
                    <input type="text" name="country" value="{{ old('country', $keyword->keyword->country) }}"
                           class="mt-1 w-full rounded-lg border-slate-300 focus:border-slate-900 focus:ring-slate-900">
                </div>

                <div>
                    <label class="block text-sm font-medium text-slate-700">CPC</label>
                    <input type="number" step="0.01" name="cpc" value="{{ old('cpc', $keyword->keyword->cpc) }}"
                           class="mt-1 w-full rounded-lg border-slate-300 focus:border-slate-900 focus:ring-slate-900">
                </div>

                <div>
                    <label class="block text-sm font-medium text-slate-700">Competition (0-100)</label>
                    <input type="number" name="competition" min="0" max="100" value="{{ old('competition', $keyword->keyword->competition) }}"
                           class="mt-1 w-full rounded-lg border-slate-300 focus:border-slate-900 focus:ring-slate-900">
                </div>

                <div class="flex items-center gap-2 pt-2">
                    <input type="checkbox" name="is_branded" value="1"
                           {{ old('is_branded', $keyword->keyword->is_branded) ? 'checked' : '' }}
